...
B<rmexp-smb|rmexp-zfs-snap [-a] [-d] [-n] [-s] -c> F<configfile>

B<rmexp-smb|rmexp-zfs-snap -l [-s] -c> F<configfile>

B<rmexp-smb|rmexp-zfs-snap --help|-?|--man>
...
will be added to statistics output. If $Conf{IncrPeriod} is set below or equal
to 0, then $Conf{FullPeriod} will be used instead.
...
45  7  *  *  2-5  /somedir/rmexp.pl -s -c /somedir/rmexp.conf | mail -Es "rmexp@`hostname` `date '+\%Y-\%m-\%d \%H:\%M'` backup warning" mbox@domain.tld

45  7  *  *  6  /somedir/rmexp.pl -c /somedir/rmexp.conf | mail -s "rmexp@`hostname` `date '+\%Y-\%m-\%d \%H:\%M'` weekly report" mbox@domain.tld
...
=head1 LICENSE and COPYRIGHT
 ...
 Copyright (C) 2011-2015 Alexander Moisseev
 All rights reserved.
